...
public class RowHelper {
    ...
    public static int drawRowsWithBackground(DrawingContext context, List<Row> rows) {
        ...
        context.guiGraphics.fill((int) context.x - 1, (int) context.y - 1, context.width / 2 + maxWidth / 2 + 1, (int) context.y + rows.size() * maxHeight, Integer.MIN_VALUE);

        int currentY = (int) context.y;
        for (Row row : rows) {
            context.guiGraphics.fill(context.width / 2 - maxWidth / 2, currentY, context.width / 2 + maxWidth / 2, currentY + getHeight(row) - 1, 553648127);
            currentY += row.render(context.of(context.x, currentY), columnMap);
        }
        return currentY;
    }
